...
export const AddDeleteList = () => {
  ...
  const clearItems = () => {
    ...
    setLiItems([]);
    setDisplay("d-block");
    setTimeout(() => setDisplay("d-none"), 2000);
  };
  ...
  const undo = () => {
    ...
    setDisplay("d-none");
    setDisplay2("d-block");
    setTimeout(() => setDisplay2("d-none"), 2000);
  };
    ...
    <div className="m-3">
      <div className={`d-flex justify-content-center ${display}`}>
        <p className={style}>
          ...
          <u style={{ cursor: "pointer" }} onClick={undo}>
            Geri al
          </u>
        </p>
      </div>
      ...
      <input type="text" className="m-1 p-1 rounded" ref={inputElementRef} />
      <button className="m-1 p-1 rounded" onClick={addItem}>
        Add
      </button>
      <button className="m-1 p-1 rounded" onClick={clearItems}>
        Clear
      </button>
      <ul>
        {liItems.map((item, index) => {
          return (
            <li key={index}>
              <span>{item}</span>
              <FaLongArrowAltUp
                className="m-1 p-1 text-info bg-success h4"
              ...
              />
              <FaTrashAlt
                className="m-1 p-1 text-light bg-danger h4"
